...
describe("SnackBarComponent", () => {
  ...
  it("renders with custom background color", () => {
    const handleClose = vi.fn();
    const msg = "Custom background color test";
    const isOpen = true;
    ...
    // Check if the snackbar has the custom background color
    const snackbar = screen.getByRole("alert");
    expect(snackbar).toHaveStyle("background-color: #1718df;");
  });

  it("renders with custom text color", () => {
    const handleClose = vi.fn();
    const msg = "Custom text color test";
    const isOpen = true;
    ...
    // Check if the snackbar has the custom text color
    const snackbar = screen.getByRole("alert");
    expect(snackbar).toHaveStyle("color: #fff;");
  });
});
