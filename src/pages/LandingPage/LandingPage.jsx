...
const LandingPage = () => {
    ...
    loginClient
      ...
      .then((res) => {
        ...
        axios({
          ...
          Cookies.set("jwtToken", respo2?.data?.results?.token, {
            secure: true,
            sameSite: "strict",
          });
      ...
      .catch((err) => {
        ...
        if (
          import.meta.env.VITE_APP_URL === "https://dev.msilsmarttaskflow.com"
        ) {
          Cookies.set(
            "jwtToken",
            "eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJLUE1HX1ByYWthc2guTmlraGlsQG1hcnV0aS5jby5pbiIsInVuaXF1ZV9pZCI6IjBkMTJiMjZiLTgyMmItNGRiYi1iNTdjLWQxMGY1NThmODNjYyIsImV4cCI6MTcxMzI4MjIxMSwiaWF0IjoxNzA3MjgyMjExLCJlbWFpbCI6IktQTUdfUHJha2FzaC5OaWtoaWxAbWFydXRpLmNvLmluIn0.VrlJMeqMhV1Tksol7XUQdLfg4rakvfZY_34G7fXKZcLFI8ms7e_oY-9LgiI2w6Gf4j-OCqdGoSY-LUrwnxlZvQ",
          ...
          Cookies.set(
            ...
            "email",
            "KPMG_Prakash.Nikhil@maruti.co.in",
            { secure: true, sameSite: "strict" }
          );
        }
      });
  ...
  return (
    <header className="bg-blue-900 flex flex-col items-stretch">
      <section
        className="flex-col overflow-hidden relative flex  w-full items-center pt-12 max-md:max-w-full"
        style={{
          width: "100vw",
          height: "100vh",
      ...
      >
        <div className="relative z-[1] flex w-full  flex-col  items-start max-md:max-w-full">
          ...
          <div
            ...
            style={{ marginTop: "5%", gap: "17vh" }}
          >
            <div className="landing-page-textbox">
              <span className="landing-page-heading">
                {" "}
                MSIL R&D <br />{" "}
              </span>
              <span
            ...
            <div className="justify-center w-full">
              <form className="gap-5 flex w-full justify-evenly">
                {userData?.results?.divisionalPMG && (
                  <CardComponent
                    title="Create / Edit Model Flow"
                    subtitle="For PMG"
                    to="/creator/modellist"
                  />
                )}
                {userData?.results?.filler && (
                  <CardComponent
                    title="Filler Activity Flow"
                    subtitle="For PIC / TL / GL"
                    to="/filler/modellist"
                  />
                )}
