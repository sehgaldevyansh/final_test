...
const MilestoneItem = ({ milestoneName, milestoneDetails, onEditDeadline, modelData, toggleAccordian, expand, paramsType, keyRefresh }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLabels(Object.keys(statemodelData?.Milestones[milestoneName]?.subTask))

                const extractedTasks = Object.keys(statemodelData?.Milestones[milestoneName]?.subTask).map((taskKey) => {
                    console.log('taskKey', taskKey)
                    const task = statemodelData?.Milestones[milestoneName]?.subTask[taskKey];
    ...
    const handleBarClick = (event, elements) => {
        if (elements.length > 0) {
            ...
            setModalPosition({ x: barPosition.x, y: barPosition.y + 20, width: barPosition.width });
    ...
    const DateChangeModal = ({ selectedBar, handleDateChange, closeModal, modalPosition, subTaskData, milestoneName }) => {
        ...
        return (
            <div className="tooltip-container" style={{ position: 'absolute', top: modalPosition?.y + 10, left: modalPosition?.x - modalPosition?.width, zIndex: 1000, overflow: 'auto' }}>
                ...
                <div className="flex"> */}
                    <div className="flex flex-col items-start border" style={{ borderRadius: '4px 0px 0px 4px' }} >
                        <LocalizationProvider dateAdapter={AdapterDayjs} sx={{}}>
                            <DemoContainer components={['DatePicker']} style={{ background: '#fff' }} sx={{

                            }}>
                                <DatePicker
                                    slotProps={{ textField: { size: "small" } }}
                                    format="DD/MM/YYYY"
                                    disabled={paramsType == "view" || paramsType == 'viewprogress' || paramsType == 'viewfull'}
                    ...
                    <div className="flex flex-col items-start border" style={{ borderRadius: '0px 4px 4px 0px' }}>
                        <LocalizationProvider dateAdapter={AdapterDayjs} sx={{}}>
    ...
    const handleDeadlineDateChange = (date, tar) => {
        ...
        onEditDeadline(
            ...
            .then(res => console.log('onEditDeadline run hua'));
    };
    ...
    return (
        <div className='flex items-center gap-2 milestone-item-container'>
            ...
            <Accordion expanded={expand ? true : false} style={{ width: '100%', boxShadow: 'none', backgroundColor: expand ? '#e6eaf4' : '#fff' }} >
                <AccordionSummary
                ...
                <AccordionDetails style={{ backgroundColor: '#f4f5f8' }}>
                    <Typography>
                        {/* <Bar data={graphData} options={options} height={'147'} width={'588'} backgroundColor={'#f4f5f8'} ref={(reference) => (chartReference.current = reference)} />
                        {modalVisible && (
                            <DateChangeModal
