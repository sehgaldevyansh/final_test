...
import { useFetchBaseModelByModelIdQuery, useFetchBlockListsQuery, useUpdateCheckedBlockListMutation, useUpdateDraftBlockListMutation } from '../../../store';
...
const FillerActivityFilling = ({ handleNext, handlePrev }) => {
  ...
  useEffect(() => {
    const filteredArray = commonUncommonFilteredData?.filter((data) => data.checkStatus === false);
  ...
  const handleSaveAsDraft = () => {
    console.log("Model Data Draft", modelData, "--", commonUncommonFilteredData);
    const filteredArray = commonUncommonFilteredData.filter((data) => data.checkStatus === false);
    if (filteredArray?.length == 0) setDisableSubmit(false)
    else { setDisableSubmit(true) }
    draftBlockList({ filteredArray: filteredArray, modelId: params?.id }).then(() => { //filteredArray
  ...
  const handleSwitchChange = (row, currentState) => {
    ...
    setCommonUncommonFilteredData((prevData) => {
      const updatedData = prevData.map((item) =>
        item.blockCode === row?.blockCode ? { ...item, checkStatus: activateValue } : item
      );
      return updatedData;
    });
  ...
  const handleVariantType = (reqBlockCode, variantName) => {
    ...
    const updatedObject = produce(commonUncommonFilteredData, (draft) => {
      const blockToUpdate = draft.find((block) => block.blockCode == reqBlockCode);

      if (blockToUpdate && blockToUpdate.variant) {
        const variantToUpdate = blockToUpdate.variant.find((variant) => variant.variantName === variantName);
  ...
  const columns = [
    {
      name: "Blocks", selector: (row) => row["blockCode"], cell: (row) => (
        <div className="" style={{ color: 'var(--base-text-color)', fontWeight: '400' }}>
          {row["blockCode"]}
        </div>
      ), sortable: true, grow: 0
    },
    // { name: 'Block Name', selector: (row) => row['blockName'], sortable: true, wrap: true, maxWidth: '20%' },
    { name: 'Base Model', selector: (row) => row['baseModel'], sortable: true, wrap: true, maxWidth: '20%' },
    {
      name: <div className="" style={{ padding: '8px 12px' }}>Choose Variants</div>, selector: (row) => row['variant'], cell: (row) => (
        <div className="flex justify-end">
          {row?.variant?.map((variant, index) => <div key={index} style={{ padding: '8px 12px', display: 'flex', justifyContent: 'flex-start', minWidth: '72px' }}>
            <Chip label={variant?.variantName} onClick={() => { !row?.checkStatus ? handleVariantType(row?.blockCode, variant?.variantName) : console.log("Not checked ") }}
              style={{
  ...
  useEffect(() => {
    const initialFilteredData = newData.filter((item) => item.variantName === selectedVariant);
  ...
  return (
    <div className=" flex flex-col items-stretch h-full justify-between" style={{
      backgroundColor: '#f4f5f8', height: '83vh',
    }}>
      <ToastContainer />
        ...
        <div className="rounded shadow bg-white flex w-full flex-col justify-center mt-4 pl-4 pr-16 py-2 items-start max-md:max-w-full max-md:pr-5">
          <div className="items-stretch flex gap-6 max-md:max-w-full max-md:flex-wrap p-2">
            ...
            <div className="items-center rounded border border-[color:var(--grey-30,#CFD2D9)] bg-white flex gap-2 pl-3  py-3 border-solid max-md:max-w-full max-md:flex-wrap max-md:pr-5 table-container">
              ...
              <div className="text-neutral-500 text-sm leading-4 tracking-tight flex items-center gap-2" style={{ width: '80%' }}>
                ...
                <select
                  ...
                  onChange={(e) => {
                    console.log('Event **', e.target.selectedIndex, e.target.value)
      ...
      <div className="flex justify-between w-full">
        ...
        <div className="flex items-center justify-between gap-2 mt-2 mb-2">
          ...
          <Button onClick={() => handlePrev(1)} className='dcms-btn-main dcms-save-as-draft-btn'>Prev</Button>
          <Button className={disableSubmit ? 'dcms-btn-main dcms-disabled-btn' : 'dcms-btn-main dcms-active-btn'} onClick={() => handleNext(1)} disabled={disableSubmit}>Next</Button>
        </div>
      </div>
