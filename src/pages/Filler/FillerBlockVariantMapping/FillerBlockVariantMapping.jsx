...
import { Box, Button, Chip, FormControl, IconButton, MenuItem, Modal, Select, TextField } from '@mui/material';
...
import {
    fetchVariantsByDynamicModel, fetchVariantBaseList, fetchVariantsByBaseModel,
    fetchVariantListByBaseAndBlock, fetchBaseModelListByBlock, fetchPartDetails
    , fetchPartListByBaseAndBlockAndVariant, fetchLevelListByBaseAndBlockAndVariant, handleReplaceVariantName, savePartLists, allPartDetails, saveSinglePartLists, handleDeletePartList
} from './dynamicApi';
...
const FillerBlockVariantMapping = ({ handleNext }) => {
    const searchOptions = [{ name: "Blocks", value: "blockCode" }, { name: "Base Model", value: 'baseModel' }];
    const navigate = useNavigate();
    const params = useParams()
    const { data, error, isLoading, refetch } = useFetchBlockListsQuery(params?.id)
    ...
    const [commonUncommonFilteredData, setCommonUncommonFilteredData] = useState(data?.data)
    const [filteredData, setFilteredData] = useState([]);
    const [searchTerm, setSearchTerm] = useState("");
    const [selectedSearchOption, setSelectedSearchOption] = useState(searchOptions[0].value);
    const [selectedOptionName, setSelectedOptionName] = useState(searchOptions[0].name)
    ...
    const [handlingNewVarientToggleSelect, sethandlingNewVarientToggleSelect] = useState();
    ...
    const handleAddRowFromMenu = async (method) => {
        ...
        handleAddRow(method);
        if (method === 'tpl') {
            const data = await fetchBaseModelListByBlock({ blockCode: queryObject?.blockCode, modelId: params?.id });
    ...
    const handleBaseModelChange = async (e, createType) => {
        ...
        if (createType === 'tpl') {
            const data = await fetchVariantListByBaseAndBlock({ blockCode: queryObject?.blockCode, modelId: params?.id, baseModel: e?.target?.value })
            setVariantList(data?.data)
        }
        setuserVariantList();
        setPartNumberList(); setuserPartNumberList();
    }
    const handleVariantChanges = async (e, createType) => {
        setuserVariantList(e.target.value);
        if (createType === 'tpl') {
            const data = await fetchPartListByBaseAndBlockAndVariant({ blockCode: queryObject?.blockCode, modelId: params?.id, baseModel: userBaseModelList, variantName: e?.target?.value })
            setPartNumberList(data?.data)
        }
        /// handling variant jugad
        setuserPartNumberList();
        if (handlingNewVarientToggleSelect !== undefined && newPartCreationMethod === 'tpl') {
            const allparts = await allPartDetails({ blockCode: queryObject?.blockCode, modelId: params?.id, baseModel: userBaseModelList, variantName: e?.target?.value });
    ...
    const handlePartChanges = async (e, createType) => {
        ...
        // }
        if (createType === 'tpl') {
            const data = await fetchPartDetails({ blockCode: queryObject?.blockCode, modelId: params?.id, baseModel: userBaseModelList, level: 1, variantName: userVariantList, partName: e?.target?.value })
    ...
    const handleLevelChanges = async (e, createType) => {
        ...
        if (createType === 'tpl') {
            const data = await fetchPartDetails({ blockCode: queryObject?.blockCode, modelId: params?.id, baseModel: userBaseModelList, level: e?.target?.value, variantName: userVariantList, partName: userPartNumberList })
    ...
    const handleSaveAsDraft = () => {
        ...
        const filteredArray = modelData.filter((data) => data.checkStatus === false);
    ...
    const handleOpen = async (row) => {
        setOpen(true);
        const varientBaseList = await fetchVariantBaseList({ row: row, modelId: params?.id });
        const data = await fetchVariantsByDynamicModel({ row: row, modelId: params?.id, varientBaseList: varientBaseList?.data });
        setQueryObject(row);
    ...
    const handleClose = async () => {
        ...
        setBaseModelList(); setUserBaseModelList();
        setVariantList(); setuserVariantList();
        setPartNumberList(); setuserPartNumberList();
        setNewPartList()
    ...
    const handleSearch = (term, option) => {
        ...
        const filteredData = data?.data?.filter((model) => {
            ...
            console.log("Property Value:", propertyValue, searchTermLowerCase, propertyValue.toLowerCase().includes(searchTermLowerCase));
    ...
    const handleVariantChange = async (variant) => {
        ...
        const data = await fetchVariantsByBaseModel({ variant, queryObject, modelId: params?.id });//queryObject
        setNewData(data?.data);
    ...
    const columns = [
        {
            name: "Blocks", selector: (row) => row["blockCode"], cell: (row) => (
                <div className="" style={{ fontWeight: '400', color: 'var(--base-text-color)' }}>
                    {row["blockCode"]}
                </div>
            ), sortable: true, grow: 0
        },
        // { name: 'Block Name', selector: (row) => row['blockName'], sortable: true, wrap: true, maxWidth: '20%' },
        { name: 'Base Model', selector: (row) => row['baseModel'], sortable: true, wrap: true, maxWidth: '15%', fontSize: '14px' },
        {
            name: <div className="" style={{ padding: '8px 12px' }}>Variants</div>, selector: (row) => row['variant'], cell: (row) => (
                <div className="flex flex-wrap">
                    {row?.variant?.map((variant, index) => <div key={index} style={{ padding: '8px 12px', display: 'flex', justifyContent: 'flex-start', minWidth: '75px' }}>
                        <Chip label={variant?.variantName}
                            ...
                            className={variant?.isModified ? `chip-modified` : 'chip-not-modified'}
                        />
                    </div>)}
                </div>
            ),
                    ...
                    >
                        ...
                        <Link className="model-name-link">
                            <i className="fi fi-rr-user">
                                <img src={pencilIcon} onClick={() => { handleOpen(row) }} alt="Pencil Icon" />
                            </i>
                        </Link>
                        {/* </Router> */}
                    </div>
    ...
    const modalColumns = [
            ...
            cell: (row) =>
                row["baseModel"] === "" ? (
                    newPartCreationMethod === "tpl" ? (
                        <select onChange={(e) => handleBaseModelChange(e, "tpl")}
                            className="dropdown-table"
                            disabled={selectedVariant === '**' || selectedVariant === '' || selectedVariant === '*'}
                        >
                            ...
                            <option key={'sadd'} value={'sdad'}>Select Base Model</option>
                        </select>
                    ) : (
                        <input
                            disabled={selectedVariant === '**' || selectedVariant === '' || selectedVariant === '*'}
                            onChange={(e) => handleBaseModelChange(e, "newPart")} className="dropdown-table" />
                    )
                ) : (
        ...
        {//variantList
            ...
            cell: (row) =>
                row["variantName"] === "" ? (
                    newPartCreationMethod === "tpl" ? (
                        <select onChange={(e) => handleVariantChanges(e, "tpl")}
                            className="dropdown-table"
                            disabled={selectedVariant === '**' || selectedVariant === '' || selectedVariant === '*'}
                        >
                            ...
                            <option key={'sadd'} value={'sdad'}>Select Variant Name</option>
                        </select>
                    ) : (
                        <input
                            defaultValue={selectedVariant}
                            disabled={(selectedVariant === '**' || selectedVariant === '' || selectedVariant === '*') || (newPartCreationMethod === 'new')}
                            onChange={(e) => handleVariantChanges(e, "newPart")} className="dropdown-table" />
                    )
                ) : (
                    row["variantName"]
                ),
        ...
        {//partNumberList
            ...
            cell: (row) =>
                row["partNo"] === "" ? (
                    newPartCreationMethod === "tpl" ? (
                        <select onChange={(e) => handlePartChanges(e, "tpl")} className="dropdown-table">
                            {partNumberList?.map((optionValue, index) => (
                                <option key={index} value={optionValue}>
                                    {optionValue}
                                </option>
                            ))}
                            <option key={'sadd'} value={'sdad'}>Select Part No.</option>
                        </select>
                    ) : (
                        <input onChange={(e) => handlePartChanges(e, "newPart")} className="dropdown-table" />
                    )
                ) : (
                    row["partNo"]
                ),
        ...
        {//levelList
            ...
            cell: (row) =>
                row["level"] === "" ? (
                    newPartCreationMethod === "tpl" ? (
                        <select onChange={(e) => handleLevelChanges(e, "tpl")} className="dropdown-table">
                            {levelList?.map((optionValue, index) => (
                                <option key={index} value={optionValue}>
                                    {optionValue}
                                </option>
                            ))}
                            <option key={'sadd'} value={'sdad'}>Select</option>
                        </select>
                    ) : (
                        <input onChange={(e) => handleLevelChanges(e, "newPart")} className="dropdown-table" />
                    )
                ) : (
                    row["level"]
                ),
        ...
        },
        {
            ...
            cell: (row) =>
                (row["level"] === "_" || newPartList) ? (
                    <div className="flex w-full py-2 gap-2 items-center justify-end" style={{}}>
                        ...
                        <div className="" style={{ cursor: "pointer", }} onClick={() => { handleDeleteRow(row.uniqueKey, row); }}>
                            <i className="">
    ...
    useEffect(() => {
        ...
        const initialFilteredData = newData?.filter((item) => item?.variantName === selectedVariant || item?.variantName === '');
    ...
    const handleNewRowSave = async () => {
        ...
        if (!createNewRef?.current?.block) {
            ...
            const swappedNewPartList = {
                ...newPartList,
                variantName: secondToLastElement?.variantName ? secondToLastElement?.variantName : newVariantName,
                referredVariant: userVariantList,
                baseModel: secondToLastElement?.baseModel ? secondToLastElement?.baseModel : baseModelCount?.data?.baseModel,
            ...
            const responsePost = await saveSinglePartLists({
                data: [swappedNewPartList], params: {
                ...
                .then((res) => { toast.success('Part Added Successfully', { position: "top-right", autoClose: 500 }) })
    ...
    const handleAddRow = (method) => {
        ...
        const updatedFilteredData = newData?.filter(item => item.variantName === selectedVariant || item.variantName === '');
    ...
    const handleDeleteRow = async (uniqueKey, row) => {
        ...
        if (uniqueKey) {
            ...
            setNewData(updatedData);
            const res = await handleDeletePartList({ body: row, modelId: params?.id });
            res.then(() => {
                toast.success("Part Deleted Success", { position: "top-right", autoClose: 500 });
    ...
    const ExpandandedRowsComponent = (suppData) => {
        ...
        const headings = ['Base Model', 'Block', 'Level', 'Parents Part No', 'DWG Type', 'Part Name', 'QTY', 'Sum QTY', 'A/AR']
        return (

            flag ? (<>
                {newPartList && <SingleRowTable baseModel={baseModelCount?.data?.baseModel} block={queryObject?.blockCode} dataDetails={newPartList} headings={headings} handleInputChange={handleCreateNewFunction} />}
                {!newPartList && <SingleRowTable baseModel={baseModelCount?.data?.baseModel} block={queryObject?.blockCode} headings={headings} handleInputChange={handleCreateNewFunction} data={['', '', '', '', '', '', '', '', '']} type={newPartCreationMethod} />}
            </>) : (<>
                <SingleRowTable baseModel={baseModelCount?.data?.baseModel} block={queryObject?.blockCode} dataDetails={suppData?.data} headings={headings} handleInputChange={handleCreateNewFunction} />
            </>)
        )
    }
    ...
    const handleAddVariant = () => {
        ...
        console.log('step 1')
        const variantExists = variantBaseList?.some(variant => variant?.variantName === handleNewVariantAddName);
    ...
    const handlePageSave = async () => {
        ...
        const res = await savePartLists({
            ...
            .then((res) => {
                toast.success('All Parts Saved Successfully', { position: "top-right", autoClose: 500 });
        ...
        if (!handleReplaceVariant) {
            const varientBaseList = await fetchVariantBaseList({ baseModel: baseModelCount?.data?.baseModel, block: queryObject?.blockCode, modelId: params?.id });
            setVariantBaseList(varientBaseList?.data)
            sethandlingNewVarientToggleSelect(0)
            const data = await fetchVariantsByDynamicModel({ baseModel: baseModelCount?.data?.baseModel, block: queryObject?.blockCode, modelId: params?.id, varientBaseList: varientBaseList?.data });
        ...
        if (handleReplaceVariant) {
            const response = await handleReplaceVariantName({
                ...
                baseModel: baseModelCount?.data?.baseModel,
            }).then(toast.success("Name changed successfully", { position: "top-right", autoClose: 500 }))
    ...
    return (
        <div className=" flex flex-col w-full items-stretch h-full" style={{
            ...
            <div className="flex-col relative flex  w-full items-center h-full modellist-container">
                    ...
                    <div className="items-stretch flex gap-6 max-md:max-w-full max-md:flex-wrap p-2">
                        <div className="items-center rounded border border-[color:var(--grey-30,#CFD2D9)] bg-white flex gap-2 px-3  py-3 border-solid max-md:max-w-full max-md:flex-wrap max-md:pr-5 ">
                            <div className="text-neutral-500 text-sm font-semibold leading-4 tracking-tight flex items-center gap-3" style={{ padding: '2px 12px' }}>
                                ...
                                <div className="flex gap-2"> <span style={{ fontWeight: '600', fontSize: '14px' }}>Total Blocks </span><span style={{ fontWeight: '400' }}>{data?.data?.length}</span></div>
                                {/* @Kaushik */}
                            </div>
                        </div>
                        <div className="items-center rounded border border-[color:var(--grey-30,#CFD2D9)] bg-white flex gap-2 pl-3  py-3 border-solid max-md:max-w-full max-md:flex-wrap max-md:pr-5 table-container">
                            ...
                            <div className="text-neutral-400 text-sm leading-4 tracking-tight flex items-center gap-2" style={{ width: '80%' }}>
                                ...
                                <select
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setSelectedOptionName(searchOptions[e.target.selectedIndex].name)
                ...
                <div className="items-stretch rounded shadow bg-white flex w-full flex-col h-full mt-6 px-4 py-5 max-md:max-w-full">
                    <div className="items-stretch rounded border border-[color:var(--grey-20,#E6E9F0)] bg-white flex flex-col pt-0.5 pb-2 px-3 border-solid max-md:max-w-full" style={{ height: '60vh', overflow: 'auto' }}  >
            ...
            <div className="flex justify-between w-full ">
                <div className="flex justify-start items-end self-end mt-2 mb-2 py-2 ">
                    <LegendComponent mainText={'Toggle condition applicable on the Variant'} legendItems={[
                        { color: '#fff', border: '2px solid var(--base-text-color)', legendText: 'Modified Variant' },
                    ]} />
                </div>
                <div className="flex items-center justify-between gap-2 mt-2 mb-2">
                    ...
                    <Button onClick={handleSaveAsDraft} className='dcms-btn-main dcms-save-as-draft-btn'>Save as Draft</Button>
                    <Button onClick={() => handleNext(0)} className='dcms-btn-main dcms-active-btn'>Next</Button>
                </div>
            </div>
            <Modal
                open={open}
            ...
            >
                <Box sx={style}>
                    ...
                    <div className="flex gap-7" style={{ fontSize: '14px', marginBottom: "14px" }}>
                        <div className="">
                            <p><span style={{ color: 'var(--base-text-color)', fontWeight: '500' }}>Block : </span><span>{queryObject?.blockCode}</span></p>
                        </div>
                        <div className="">
                            <p><span style={{ color: 'var(--base-text-color)', fontWeight: '500' }}>Base Model : </span><span>{queryObject?.baseModel}</span></p>
                        </div>
                        <div className=""></div>
                    </div>
                    <div className="flex flex-col  items-center justify-start border" style={{
                        backgroundColor: '#fff',
                        borderRadius: '12px',
                        minHeight: '95%',
                        width: '100%'
                        ...
                        <div className="mt-4 flex flex-row w-full px-8 ">
                            <div className="flex  justify-end w-full items-center">
                                <StepperComponent
                                    variantList={variantBaseList?.map((data) => data?.variantName)}
                                    baseModelList={variantBaseList?.map((data) => (data?.referredBaseModel && data?.referredBaseModel != 'null') ? data?.referredBaseModel : data?.baseModel)}
                                    onChange={handleVariantChange}
                                    keyRefresh={handlingNewVarientToggleSelect}
                                />
                            </div>
                            {/* ///// add variant */}
                            <div className="flex items-center  justify-end gap-2" style={{ width: '25%' }}>
                                <FormControl>
                                    <Select
                                        value=""
                                        displayEmpty
                                        renderValue={() => (
                                            <div className="flex gap-2 items-center" style={{ color: isVariantDropdownOpen ? '#fff' : 'inherit' }} title={selectedVariant === '**' ? 'Name the variant first' : ''}>
                                                <div className="flex">
                                                    <i className="">
                                                        <img src={isVariantDropdownOpen ? addIconWhite : addIcon} alt="Add Icon" />
                                                    </i>
                                                </div>
                                                <div className="" style={{ color: isVariantDropdownOpen ? '#fff' : '#171c8f' }} >
                                                    {selectedVariant === '**' ? 'Variant Locked' : 'Add Variant'}
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        disabled={selectedVariant === '**' || selectedVariant === '' || selectedVariant === '*'} // Disable the dropdown if the selected variant is '**'

                                        sx={{
                                            cursor: 'pointer',
                                            backgroundColor: isVariantDropdownOpen ? '#171c8f' : 'inherit',
                                        ...
                                        size='small'
                                    >
                                        <MenuItem sx={{ color: "#66696B", '&:hover': { borderRadius: "6px", backgroundColor: '#A7A7A7', color: '#fff' } }} onClick={() => { console.log('yahan aaya'); return handleAddRowFromMenuVariant('tpl') }}>
                                            Create From TPL
                                        </MenuItem>
                                ...
                                <div className="">
                                    <Button onClick={handlePageSave}
                                        ...
                                        <div className="flex gap-2 items-center" style={{ color: isVariantDropdownOpen ? '#fff' : 'inherit' }}>
                                            <div className="flex">
                                                <i className="">
                                                    <img src={saveIcon} alt="Save Icon" />
                                                </i>
                                            </div>
                                            <div className="" style={{ color: '#171c8f', textTransform: 'none' }} >
                                                Save
                                            </div>
                                        </div>
                                    </Button>
                        ...
                        <div className="" style={{
                            ...
                            <div className="flex items-center justify-between w-full gap-4 border shadow" style={{ padding: '16px 22px', borderTop: "3px solid #171c8f", marginTop: '0px' }}>
                                <div className="flex items-center justify-between gap-8">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <p style={{ fontSize: '14px' }}>{handlingNewVarientToggleSelect ? 'Variant Name' : 'Change Variant Name'}</p>
                                        {handlingNewVarientToggleSelect && <TextField
                                            size='small'
                                            ...
                                            onChange={(e) => {
                                                ...
                                                const isVariantNameUnique = inputVariantName === '' || !variantBaseList.some((variant) => variant?.variantName === inputVariantName);
                                                if (isVariantNameUnique) {
                                                    setHandleNewVariantAddName(inputVariantName);
                                                    const updatedVariantList = variantBaseList?.map((variant) => {
                                                    ...
                                                    setErrorMessage("Please select unique and valid variant name")//////////////
                                                }
                                            }}
                                        />}
                                        {!handlingNewVarientToggleSelect && <TextField
                                        ...
                                        />}
                                        {(selectedVariant === '**' || selectedVariant === '' || selectedVariant === '*') && <p className='text-red-700'>Please type a valid variant name</p>}
                                ...
                                <div className="flex items-center gap-2">
                                    <FormControl>
                                        <Select
                                            ...
                                            displayEmpty
                                            renderValue={() => (
                                                <div className="flex gap-2 items-center" style={{ color: isDropdownOpen ? '#fff' : 'inherit' }}>
                                                    <div className="flex">
                                                        <i className="">
                                                    ...
                                                    <div className="" style={{ color: '#fff' }}>Add Part</div>
                                        ...
                                        >
                                            ...
                                            <MenuItem sx={{ color: "#66696B", '&:hover': { borderRadius: "6px", backgroundColor: '#A7A7A7', color: '#fff' } }} onClick={() => handleAddRowFromMenu('tpl')}>
                                                Create From TPL
                                            </MenuItem>
                                            <MenuItem sx={{ color: "#66696B", '&:hover': { borderRadius: "6px", backgroundColor: '#A7A7A7', color: '#fff' } }} onClick={() => handleAddRowFromMenu('new')}>
                                                Create New
                                            </MenuItem>
                                        </Select>
                            ...
                            <div className="w-full border" style={{ overflow: 'auto', height: '45vh' }}>
                                <DataTable
