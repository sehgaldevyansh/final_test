...
import { SavePartInformation, fetchCauseOfChange, fetchImportanceRank, fetchLevelOfChange, fetchPartDetailsByDynamicModel } from './apis'
import {
...
const FillerGeneralInfo = ({ handleNext, handlePrev }) => {
  ...
  const { data: baseModelCount, refetch } = useFetchBaseModelByModelIdQuery(params?.id)
  ...
  const handleSaveAsDraftButton = async () => {
    const data = await SavePartInformation({
      body: {
        ...
        "selectedVariant": selectedVariant.variantName ? selectedVariant.variantName : "",
        "selectedPart": selectedPart ? selectedPart : "",
        isFilled: ((SDM && specNonspec && causeOfChange && levelOfChange && causeOfModification && importanceRank && newRmUsage && maruApplicability && selectedSystem && selectedBlock && selectedVariant && selectedPart) ? true : false),
        "rowStateFlag": 1
      },
      params: {
    ...
    }).then((res) => { toast.success('Part Details Saved'); refetch() })
  ...
  const handlePartClick = async (part) => {
    ...
    const data = await fetchPartDetailsByDynamicModel({ baseModel: baseModelCount, systemName: selectedSystem, blockName: selectedBlock?.blockName, variantName: selectedVariant?.variantName, partNumber: part, modelId: params?.id })
  ...
  return (
    <div className=" flex flex-col items-stretch h-full" style={{
      ...
      <div className="flex-col relative flex  w-full items-center max-md:max-w-full max-md:pl-5  h-full modellist-container gap-6">
          ...
          <div className="items-stretch flex gap-6 max-md:max-w-full max-md:flex-wrap p-2">
            <div className="items-center rounded border border-[color:var(--grey-30,#CFD2D9)] bg-white flex gap-2 px-3  py-3 border-solid max-md:max-w-full max-md:flex-wrap max-md:pr-5 ">
              <div className="text-neutral-500 text-sm font-semibold leading-4 tracking-tight flex items-center gap-3" style={{ padding: '2px 12px' }}>
                ...
                <div className="flex gap-2 items-center" style={{ fontWeight: '600' }}>Blocks Filled  <span style={{ fontWeight: '400' }} >{((baseModelCount?.data?.totalBlocksFilled / baseModelCount?.data?.totalBlocks) * 100).toFixed(0)}%</span><CircularProgress variant="determinate" value={((baseModelCount?.data?.totalBlocksFilled / baseModelCount?.data?.totalBlocks) * 100).toFixed(0)} thickness={22} size='1rem' sx={{ backgroundColor: '#ECECEC', borderRadius: '50%', color: '#FF7C02' }} /></div>
                {/* @Kaushik */}
              </div>
            </div>
        ...
        >
          <div className="bg-white shadow h-full" style={{ borderRadius: '4px', maxHeight: '550px', overflow: "auto", width: "260px" }}>
            <Sidebar
              data={filteredData}
          ...
          {selectedPart !== null ? (<div className="rounded shadow bg-white flex grow flex-col w-full p-4 h-full">
            <div className="flex flex-col border rounded w-full">
              ...
              <div className="">
                {singleRowData && <SingleRowTable dataDetails={singleRowData} headings={['Base Model', 'Block', 'Level', 'Parents Part No', 'DWG Type', 'Part Name', 'QTY', 'Sum QTY', 'A/AR']} data={['Y0M11', 'CS510-', '01', 'Y0M11CS510-001', 'S', 'LATCH ASSY,FRONT DOOR,R', '001', '001', 'A']} />}
              </div>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div className="items-stretch self-stretch rounded border border-[color:var(--Grey-20,#E6E9F0)] bg-white flex  flex-col justify-center p-4 border-solid gap-4">
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: "space-between", marginTop: '10px' }}>
                      ...
                      <Select onChange={(e) => { setSDM(e?.target?.value) }}
                        // defaultValue={singleRowData?.sdm !== null ? singleRowData?.sdm : "Select"} 
                        value={SDM}
                        labelId="sdm-label" id="sdm" style={{ minWidth: "50%", height: "32px", borderRadius: "4px" }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: "space-between", marginTop: '10px' }}>
                      ...
                      <Select onChange={(e) => { setSpecNonspec(e?.target?.value) }}
                        // defaultValue={singleRowData?.specNonSpec !== null ? singleRowData?.specNonSpec : "Select"} 
                        value={specNonspec} labelId="spec-label" id="spec" style={{ minWidth: "50%", height: "32px", borderRadius: "4px" }}>
                        <MenuItem value="Select">Select</MenuItem>
                        <MenuItem value="Yes">Yes</MenuItem>
                        <MenuItem value="No">No</MenuItem>
                      </Select>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div className="items-stretch self-stretch rounded border border-[color:var(--Grey-20,#E6E9F0)] bg-white flex  flex-col justify-center p-4 border-solid gap-4">
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: "space-between", marginTop: '10px' }}>
                      ...
                      <Select onChange={(e) => { setLevelOfChange(e?.target?.value) }}
                        // defaultValue={singleRowData?.levelOfChange !== null ? singleRowData?.levelOfChange : ""} 
                        value={levelOfChange}
                        labelId="level-label" id="level" style={{ minWidth: "50%", height: "32px", borderRadius: "4px" }} MenuProps={{
                          PaperProps: {
                            sx: {
                              // bgcolor: 'pink',
                              '& .MuiMenuItem-root': {
                        ...
                        {levelDropdown?.map((optionValue, index) => {
                          console.log('check if match', optionValue === singleRowData?.levelOfChange)
                          console.log('optionss', optionValue, singleRowData?.levelOfChange)
                          return (
                            <MenuItem key={index} value={optionValue} style={{ whiteSpace: 'normal' }}>
                              {optionValue}
                            </MenuItem>
                          )
                        }
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: "space-between", marginTop: '10px' }}>
                      ...
                      <Select onChange={(e) => { setCauseOfModification(e?.target?.value) }}
                        // defaultValue={singleRowData?.causeOfModification !== null ? singleRowData?.causeOfModification : ""} 
                        value={causeOfModification}
                        labelId="cause-label" id="cause" style={{ minWidth: "50%", height: "32px", borderRadius: "4px" }} MenuProps={{
                          PaperProps: {
                            sx: {
                              // bgcolor: 'pink',
                              '& .MuiMenuItem-root': {
                        ...
                        <MenuItem value="" style={{ whiteSpace: 'normal' }}>Select</MenuItem>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: "space-between", marginTop: '10px' }}>
                      ...
                      <Select onChange={(e) => { setImportanceRank(e?.target?.value) }}
                        // defaultValue={singleRowData?.importanceRank !== null ? singleRowData?.importanceRank : ""} 
                        value={importanceRank}
                        labelId="importance-label" id="importance" style={{ minWidth: "50%", height: "32px", borderRadius: "4px" }} MenuProps={{
                          PaperProps: {
                            sx: {
                              // bgcolor: 'pink',
                              '& .MuiMenuItem-root': {
                        ...
                        {rankDropdown?.map((optionValue) => (
                          <MenuItem key={optionValue} value={optionValue} style={{ whiteSpace: 'normal' }}>
                            {optionValue}
                          </MenuItem>
                        ))}
                      </Select>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: "space-between", marginTop: '10px' }}>
                      ...
                      <Select onChange={(e) => { setNewRmUsage(e?.target?.value) }}
                        // defaultValue={singleRowData?.newRmUsage !== null ? singleRowData?.newRmUsage : ""} 
                        labelId="agreement-label" id="agreement" style={{ minWidth: "50%", height: "32px", borderRadius: "4px" }} value={newRmUsage}>
                        {/* <MenuItem value="">Select</MenuItem> */}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: "space-between", marginTop: '10px' }}>
                      ...
                      <Select onChange={(e) => { setMaruApplicability(e?.target?.value) }}
                        // defaultValue={singleRowData?.maruApplicability !== null ? singleRowData?.maruApplicability : ""} 
                        labelId="maruA-label" id="maruA" style={{ minWidth: "50%", height: "32px", borderRadius: "4px" }} value={maruApplicability}>
                        {/* <MenuItem value="">Select</MenuItem> */}
                        <MenuItem value="applicable">Applicable</MenuItem>
                        <MenuItem value="nonApplicable">Non Applicable</MenuItem>
                      </Select>
                    </div>
                  </div>
                </div>
          ...
          </div>) : <div className="rounded shadow bg-white flex grow flex-col w-full p-4 h-full" style={{ color: 'var(--base-text-color)', fontWeight: '600' }}>Please Choose a Part To Continue</div>}
      ...
      <div className="flex justify-between w-full">
        ...
        <div className="flex items-center justify-between gap-2 mt-2 mb-2">
          ...
          <Button onClick={handleSaveAsDraftButton} className='dcms-btn-main dcms-save-as-draft-btn'>Save as Draft</Button>
          <Button onClick={() => handlePrev(2)} className='dcms-btn-main dcms-save-as-draft-btn'>Prev</Button>
          <Button onClick={() => handleNext(2)} disabled={parseInt(((baseModelCount?.data?.totalBlocksFilled / blockLength?.data?.length) * 100).toFixed(0)) != 100} className={parseInt(((baseModelCount?.data?.totalBlocksFilled / blockLength?.data?.length) * 100).toFixed(0)) != 100 ? "dcms-btn-main dcms-disabled-btn" : "dcms-btn-main dcms-active-btn"}>Next</Button>
        </div>
      </div>
    </div>
