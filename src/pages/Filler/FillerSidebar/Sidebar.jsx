...
import {
  fetchPartByDynamicModel,
  fetchBlocksByDynamicModel,
  fetchVariantByDynamicModel,
  fetchSystemByDynamicModel,
} from "./apis";
...
const Sidebar = ({
  ...
  onSystemClick,
  handleSearch,
  showVariantsAndParts = true,
  draftStatus,
}) => {
  // console.log("sidebardata",data);
  const params = useParams();
  const { data: baseModelInformation } = useFetchBaseModelByModelIdQuery(
    params?.id
  );
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (myObjectArray) {
        handleSystemClick(myObjectArray[0]?.systemName);
      }
      //done
    }
    fetchData();
  ...
  const handleSystemClick = async (systemName) => {
    ...
    const response = await fetchSystemByDynamicModel({
  ...
  const handleBlockClick = async (block) => {
    ...
    const response = await fetchVariantByDynamicModel({
      baseModel: baseModelInformation,
  ...
  const handleVariantClick = async (variant) => {
    ...
    const response = await fetchPartByDynamicModel({
      baseModel: baseModelInformation,
      systemName: selectedSystem,
      modelId: params?.id,
  ...
  return (
    <List style={{ paddingTop: 0, borderRadius: "4px" }}>
      <ListItem
        className="sidebar-list-item-header"
        style={{
      ...
      >
        <ListItemText
          primary={<span className="custom-system-name">BLOCKS</span>}
        />
      </ListItem>
      {selectedSystemList?.map((system, index) => (
        <div key={index}>
          ...
          >
            <ListItemText
              primary={
                <span className="custom-system-name">{system?.systemName}</span>
              }
            />
                ...
                <div key={block?.blockName}>
                  ...
                  >
                    {showVariantsAndParts && ( // Check if variants and parts are shown
                      <ListItemText
                        primary={
                          <span className="sidebar-list-item-child">
                            {block.blockName}
                          </span>
                        }
                      />
                        ...
                        {block?.isFilled && ( // If not showing variants and parts, display tick icon
                          <>
                            <DoneIcon
                              style={{
                                marginLeft: "5px",
                                color: block?.isFilled ? "#219c27" : "#171c8f",
                                marginRight: "5px",
                              }}
                            />
                          </>
...

                  {
                    ...
                    >
                      ...
                      <Collapse
                        in={showVariantsAndParts && selectedVariant !== null}
                        timeout="auto"
                        unmountOnExit
                      >
                        <List style={{ paddingTop: "0px" }}>
                          {showVariantsAndParts &&
                            selectedVariant &&
                            selectedPartList?.map((part, index) => {
                              ...
                              return (
                                <React.Fragment key={index}>
                                  <ListItem
                                    disabled={!showVariantsAndParts}
                                    className={`sidebar-list-item-part ${
                                  ...
                                  {index < selectedPartList.length - 1 && (
                                    <Divider />
                                  )}
                                </React.Fragment>
                              );
