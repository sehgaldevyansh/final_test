...
const FillerModellist = () => {
  ...
  const handleSearch = (term, option) => {
    ...
    const filteredData = modelData?.filter((model) => {
      ...
      console.log(
        "model[option?.toLowerCase()]?.toString()",
        model[option]?.toString()
      );
      const propertyValue =
        option === "editedBy"
          ? model[option]?.toString()
  ...
  const columns = useMemo(
    () => [
      ...
      {
        name: "Status",
        selector: (row) => row?.rowStateFlag,
        cell: (row) => (
          <>
            {
              statusMap.find((item) => item.statusId === row?.rowStateFlag)
                ?.statusText
            }
          </>
        ),
