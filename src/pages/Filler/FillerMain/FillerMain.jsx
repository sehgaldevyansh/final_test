...
const tabList = ['Step 1: CREATE PARTLIST', 'Step 2: COMMON/UN-COMMON MAPPING', 'Step 3: GENERAL INFO', 'Step 4: FILL DEVIATION', 'Step 5: PREVIEW']
...
const FillerMain = () => {
    ...
    const handlePrev = (stepNo) => {
        console.log('step no coming in prev', stepNo, stepNo === 0 ? stepNo : (stepNo - 1))
        setValue(stepNo === 0 ? stepNo : (stepNo - 1))
    }

    const fillerPages = [
        <FillerBlockVariantMapping key={0} handleNext={handleNext} />,
        <FillerActivityFilling key={1} handleNext={handleNext} handlePrev={handlePrev} />,
        <FillerGeneralInfo key={2} handleNext={handleNext} handlePrev={handlePrev} />,
    ...
    return (
        <div className=" flex flex-col items-stretch h-full" style={{
            ...
            <div className="modal-list-main-container">
                <NavigationPathComponent paths={[{ name: 'DCMS Models', path: '/filler/modellist' }]} current='DCMS Activity Filling' /></div>
                        ...
                        <div style={{ borderBottom: 1, borderColor: 'divider' }}>
                            <Tabs sx={{
                                ...
                                {tabList?.map((tab, index) => {
                                    return (
                                        <Tab key={index} label={tab} {...a11yProps(index)} disabled={value !== index} sx={{ textTransform: 'none', fontSize: '12px', fontWeight: "400" }} />
                                    )
                                })}
                            </Tabs>
                        </div>
