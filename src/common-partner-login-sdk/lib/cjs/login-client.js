...
class LoginClient {
        ...
        return new Promise((resolve, reject) => {
            fetch(url).then((env_data) => {
                env_data.json().then(data => {
                    ...
                    aws_amplify_1.default.configure({ Auth: LoginClient.auth });
                    resolve(true);
                });
            }, (env_data_error) => {
    ...
    sendOtp(contactInfo) {
        ...
        return new Promise((resolve, reject) => {
            aws_amplify_1.Auth.signIn(contactInfo).then((data) => {
    ...
    signIn(username, password) {
        return new Promise((resolve, reject) => {
            aws_amplify_1.Auth.signIn(username, password).then((data) => {
    ...
    createNewPassword(password) {
        return new Promise((resolve, reject) => {
            aws_amplify_1.Auth.completeNewPassword(this.signInSession, password).then((data) => {
                resolve({
                    auth_status: 'TOKEN_RECEIVED',
                    data: data
                });
    ...
    changePassword(oldPassword, newPassword) {
        return new Promise((resolve, reject) => {
            aws_amplify_1.Auth.currentAuthenticatedUser()
                .then(user => {
                aws_amplify_1.Auth.changePassword(user, oldPassword, newPassword).then(data => {
                    resolve({
    ...
    forgotPassword(username) {
        return new Promise((resolve, reject) => {
            aws_amplify_1.Auth.forgotPassword(username)
                .then(data => {
                resolve({
    ...
    submitforgotPasswordOtp(username, code, newPassword) {
        return new Promise((resolve, reject) => {
            aws_amplify_1.Auth.forgotPasswordSubmit(username, code, newPassword)
                .then(data => {
                resolve({
    ...
    getToken() {
        ...
        if (window && window.localStorage) {
            return new Promise((resolve, reject) => {
                setTimeout(() => {
                    aws_amplify_1.Auth.currentAuthenticatedUser().then((authUser) => {
    ...
    refreshSession() {
        return new Promise((resolve, reject) => {
            try {
                aws_amplify_1.Auth.currentAuthenticatedUser().then((user) => {
                    aws_amplify_1.Auth.currentSession().then(currentSession => {
    ...
    verifyCognitoOtpChallenge(otp) {
        return new Promise((resolve, reject) => {
            const cognitoSession = aws_amplify_1.Auth.sendCustomChallengeAnswer(this.signInSession, otp).then((data) => {
                if (data.Session) {
                    reject({
        ...
        return new Promise((resolve, reject) => {
            ...
            fetch(url, {
                ...
                otp_verify_data.json().then(data => {
                    if (!data.error) {
                        aws_amplify_1.Auth.signIn(this.authContact).then((data) => {
                            aws_amplify_1.Auth.sendCustomChallengeAnswer(data, this.secretAuthCode).then((data) => {
                                resolve({
                                    otp_status: 'OTP_VERIFIED',
                                    auth_status: 'TOKEN_RECEIVED',
                                    data: data
...
exports.loginClient = new LoginClient();
